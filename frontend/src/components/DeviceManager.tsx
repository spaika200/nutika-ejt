          ...
          {!isAdding ? (
                ...
                {devices.length === 0 ? (
                  ...
                  devices.map(device => (
                    <div key={device.id} className="bg-slate-900/50 border border-slate-700 p-4 rounded-xl flex justify-between items-center">
                      <div>
                        <h4 className="font-semibold text-slate-200">{device.name}</h4>
            ...
            <form onSubmit={handleAddDevice} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-700 mt-6">
                <button type="button" onClick={() => setIsAdding(false)} className="flex-1 px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-white transition-colors">Cancel</button>
